...
test('home install switcher and browse CTA work', async ({ page }) => {
  ...
  await expect(page.getByText('npx clawhubsg@latest install sonoscli')).toBeVisible()

  await page.getByRole('tab', { name: 'pnpm' }).click()
  await expect(page.getByText('pnpm dlx clawhubsg@latest install sonoscli')).toBeVisible()

  await page.getByRole('tab', { name: 'bun' }).click()
  await expect(page.getByText('bunx clawhubsg@latest install sonoscli')).toBeVisible()
